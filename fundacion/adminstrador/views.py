...
from adminstrador.forms import AcudeInstitucionForm, EstadoForm, PaisForm, EstadoCivilForm, EstatusForm, \
    ...
    AgresorForm, MedioComunicacionForm, ComoSeEnteroForm, EstadoMentalForm
from config.models import AcudeInstitucion, Estado, Pais, EstadoCivil, Estatus, LenguaIndigena, MedioContacto, \
    ...
    MedioComunicacion, ComoSeEntero, EstadoMental
...
class AyudaAdd(PermissionRequiredMixin, CreateView):
    ...
    model = Ayuda
    template_name = 'config/formulario_1Col.html'
    success_url = '/administrador/ayuda/list'
    form_class = AyudaForm

    def get_context_data(self, **kwargs):
        context = super(AyudaAdd, self).get_context_data(**kwargs)
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
            context['titulo'] = 'Agregar un ayuda'
        if 'instrucciones' not in context:
            context['instrucciones'] = 'Completa todos los campos para registrar un'
        return context
...
@permission_required(perm='catalogo', login_url='/')
def list_ayuda(request):
    template_name = 'administrador/tab_ayuda.html'
    return render(request, template_name)


class AyudaAjaxList(PermissionRequiredMixin, BaseDatatableView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    ...
    def render_column(self, row, column):
        ...
        if column == 'editar':
            return '<a class="" href ="' + reverse('administrador:edit_ayuda',
                                                   kwargs={
                                                       'pk': row.pk}) + '"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/editar.png"></a>'
        elif column == 'eliminar':
            return '<a class=" modal-trigger" href ="#" onclick="actualiza(' + str(
        ...
        return super(AyudaAjaxList, self).render_column(row, column)

    def get_initial_queryset(self):
        return Ayuda.objects.all()
...
class AyudaEdit(PermissionRequiredMixin, UpdateView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    success_url = '/administrador/ayuda/list'
    ...
    def get_context_data(self, **kwargs):
        context = super(AyudaEdit, self).get_context_data(**kwargs)
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
            context['titulo'] = 'Editar '
...
class MedioComunicacionAdd(PermissionRequiredMixin, CreateView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    ...
    template_name = 'config/formulario_1Col.html'
    success_url = '/administrador/medio_comunicacion/list'
    form_class = MedioComunicacionForm

    def get_context_data(self, **kwargs):
        context = super(MedioComunicacionAdd, self).get_context_data(**kwargs)
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
            context['titulo'] = 'Agregar un medio_comunicacion'
        if 'instrucciones' not in context:
            context['instrucciones'] = 'Completa todos los campos para registrar un'
        return context
...
@permission_required(perm='catalogo', login_url='/')
def list_medio_comunicacion(request):
    template_name = 'administrador/tab_medio_comunicacion.html'
    return render(request, template_name)


class MedioComunicacionAjaxList(PermissionRequiredMixin, BaseDatatableView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    ...
    def render_column(self, row, column):
        ...
        if column == 'editar':
            return '<a class="" href ="' + reverse('administrador:edit_medio_comunicacion',
                                                   kwargs={
                                                       'pk': row.pk}) + '"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/editar.png"></a>'
        elif column == 'eliminar':
            return '<a class=" modal-trigger" href ="#" onclick="actualiza(' + str(
        ...
        return super(MedioComunicacionAjaxList, self).render_column(row, column)

    def get_initial_queryset(self):
        return MedioComunicacion.objects.all()
...
class MedioComunicacionEdit(PermissionRequiredMixin, UpdateView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    success_url = '/administrador/medio_comunicacion/list'
    ...
    def get_context_data(self, **kwargs):
        context = super(MedioComunicacionEdit, self).get_context_data(**kwargs)
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
            context['titulo'] = 'Editar '
