    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Consejero', 'url': reverse('administrador:list_consejero')}]
            context['rutas']= rutas
...
class SupervisorAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Supervisor', 'url': reverse('administrador:list_supervisor')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class EstadoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse('administrador:list_estado', kwargs={'pais': self.kwargs['pais']})


@permission_required(perm='catalogo', login_url='/')
def list_estado(request, pais):
    context = {'pais':pais}
    ...
    def render_column(self, row, column):
        ...
        if column == 'editar':
            return '<a class="" href ="' + reverse('administrador:edit_estado',
                                                   kwargs={
                                                       'pk': row.pk, 'pais': self.kwargs['pais']}) + '"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/editar.png"></a>'
        elif column == 'eliminar':
            return '<a class=" modal-trigger" href ="#" onclick="actualiza(' + str(
                row.pk) + ')"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/eliminar.png"></a>'
        elif column == 'id':
            ...
            return '<a class="" href ="' + reverse('administrador:list_municipio',
                                                   kwargs={
                                                       'estado': row.pk}) + '"><i class="material-icons">account_balance</i></a>'
...
class EstadoEdit(PermissionRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        estado = self.model.objects.get(pk=self.kwargs['pk'])
        form = EstadoForm(request.POST, request.FILES, instance=estado)
        if form.is_valid():
            estado_form = form.save()
            return HttpResponseRedirect(self.get_success_url())
        else:
            return self.render_to_response(self.get_context_data(form=form))
...
class PaisAjaxList(PermissionRequiredMixin, BaseDatatableView):
    ...
    model = Pais
    columns = ['id', 'nombre', 'estados', 'editar', 'eliminar']
    order_columns = ['id', 'nombre']
    max_display_length = 100
    ...
    def render_column(self, row, column):
        ...
        if column == 'editar':
            ...
            return '<a class="" href ="' + reverse('administrador:list_estado',
                                                   kwargs={
                                                       'pais': row.pk}) + '"><i class="material-icons">landscape</i></a>'
...
class EstadoCivilAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Estado civil', 'url': reverse('administrador:list_estado_civil')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class MedioContactoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Forma de contacto', 'url': reverse('administrador:list_medio_contacto')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class ModalidadViolenciaAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Modalidad de violencia', 'url': reverse('administrador:list_modalidad_violencia')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class MunicipioAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            ...
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'País', 'url': reverse('administrador:list_pais')},
                     {'nombre': 'Estado', 'url': reverse('administrador:list_estado', kwargs={'pais': estado.pais.pk})},
                     {'nombre': 'Municipio', 'url': reverse('administrador:list_municipio', kwargs={'estado': estado.pk})}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
    ...
    def get_success_url(self):
        return reverse('administrador:list_municipio', kwargs={'estado': self.kwargs['estado']})
    ...
    def render_column(self, row, column):
        ...
        if column == 'editar':
            return '<a class="" href ="' + reverse('administrador:edit_municipio',
                                                   kwargs={
                                                       'pk': row.pk, 'estado':self.kwargs['estado']}) + '"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/editar.png"></a>'
        elif column == 'eliminar':
            return '<a class=" modal-trigger" href ="#" onclick="actualiza(' + str(
                row.pk) + ')"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/eliminar.png"></a>'
        elif column == 'id':
...
class MunicipioEdit(PermissionRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        municipio = self.model.objects.get(pk=self.kwargs['pk'])
        form = MunicipioForm(request.POST, request.FILES, instance=municipio)
        if form.is_valid():
            municipio_form = form.save()
            return HttpResponseRedirect(self.get_success_url())
        else:
            return self.render_to_response(self.get_context_data(form=form))
...
class ColoniaAdd(PermissionRequiredMixin, CreateView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
            context['titulo'] = 'Agregar una colonia'
        if 'instrucciones' not in context:
            context['instrucciones'] = 'Completa todos los campos para registrar un'
        return context
    ...
    def get_success_url(self):
        return reverse('administrador:list_colonia', kwargs={'municipio': self.kwargs['municipio']})
...
class ColoniaAjaxList(PermissionRequiredMixin, BaseDatatableView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    ...
    def render_column(self, row, column):
        ...
        if column == 'editar':
            return '<a class="" href ="' + reverse('administrador:edit_colonia',
                                                   kwargs={
                                                       'pk': row.pk, 'municipio':self.kwargs['municipio']}) + '"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/editar.png"></a>'
        elif column == 'eliminar':
            return '<a class=" modal-trigger" href ="#" onclick="actualiza(' + str(
                row.pk) + ')"><img  src="http://orientacionjuvenil.colorsandberries.com/Imagenes/fundacion_origen/3/eliminar.png"></a>'
        elif column == 'id':
...
class ColoniaEdit(PermissionRequiredMixin, UpdateView):
    redirect_field_name = 'next'
    login_url = '/'
    permission_required = 'catalogo'
    success_url = '/administrador/colonia/list'
...
class NivelEstudioAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Nivel de estudios', 'url': reverse('administrador:list_nivel_estudio')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class NivelViolenciaAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Nivel de violencia', 'url': reverse('administrador:list_nivel_violencia')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class OcupacionAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Ocupación', 'url': reverse('administrador:list_ocupacion')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class ReligionAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Religión', 'url': reverse('administrador:list_religion')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class TipoViolenciaAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Tipo de violencia', 'url': reverse('administrador:list_tipo_violencia')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class ViolentometroAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Violentómetro', 'url': reverse('administrador:list_violentometro')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class ViveConAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Vive con', 'url': reverse('administrador:list_vive_con')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class SexoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Sexo', 'url': reverse('administrador:list_sexo')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class MotivoLLamadaAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Motivo de llamada', 'url': reverse('administrador:list_motivo_llamada')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class DependenciaAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Dependencia de instituciones', 'url': reverse('administrador:list_dependencia')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class RedesApoyoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Redes de apoyo', 'url': reverse('administrador:list_redes_apoyo')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class VictimaInvolucradaAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Víctimas involucradas', 'url': reverse('administrador:list_victimas_involucradas')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class AgresorAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Agresor', 'url': reverse('administrador:list_agresor')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class ComoSeEnteroAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Cómo se enteró', 'url': reverse('administrador:list_como_se_entero')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class NivelRiesgoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Nivel de riesgo', 'url': reverse('administrador:list_nivel_riesgo')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class RecomendacionRiesgoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Recomendaciónes para vida en riesgo', 'url': reverse('administrador:list_recomendacion_riesgo')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class FaseCambioAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Fase de cambio', 'url': reverse('administrador:list_fase_cambio')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class AliadoAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Aliado', 'url': reverse('administrador:list_aliado')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class ActividadUsuarioAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Actividades de usuarios', 'url': reverse('administrador:list_actividad_usuario')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class TipificacionAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Tipificación', 'url': reverse('administrador:list_tipificacion')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            ...
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Tipificaciones', 'url': reverse('administrador:list_tipificacion')},
                     {'nombre': 'Categorías', 'url': reverse('administrador:list_categoria_tipificacion', kwargs={'tipificacion':cat_tipificacion.tipificacion.pk})},
                     {'nombre': 'Subcategorías', 'url': reverse('administrador:list_subcategoria_tipificacion', kwargs={'categoria_tipificacion':cat_tipificacion.pk})}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
...
class TutorAdd(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'rutas' not in context:
            rutas = [{'nombre': 'menu', 'url': reverse('webapp:index')},
                     {'nombre': 'Catálogos', 'url': reverse('administrador:catalogos')},
                     {'nombre': 'Tutor', 'url': reverse('administrador:list_tutor')}]
            context['rutas']= rutas
        if 'form' not in context:
            context['form'] = self.form_class()
        if 'titulo' not in context:
